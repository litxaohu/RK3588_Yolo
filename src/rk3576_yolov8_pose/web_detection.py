...
def load_classes(path):
    pass # Pose 模型默认只有 'person'，不支持动态类别
            ...
            while not self._stop_event.is_set():
                ...
                if self.model and self.co_helper:
                    ...
                    if outputs is not None:
                        ...
                        boxes = post_process_with_thresh(outputs, obj, nms)
                        if boxes is not None and len(boxes) > 0:
                            draw_pose(frame, boxes, self.co_helper)
...
async def predict(
    ...
    try:
        ...
        if boxes is not None and len(boxes) > 0:
            # 在返回的原始帧上画图 (针对视频流显示)
            draw_pose(img, boxes, _global_co_helper)
        ...
        # if boxes is not None and len(boxes) > 0:
        #     print("\n========== YOLOv8-Pose Detection Results ==========", flush=True)
        #     for i, box in enumerate(boxes):
        #         real_box = _global_co_helper.get_real_box(box)
        #         real_kp = _global_co_helper.get_real_keypoint(box.keypoint)
        #         class_name = CLASSES[box.classId] if box.classId < len(CLASSES) else f"Class {box.classId}"
        #         
        #         print(f"  - Obj {i}: Class={class_name}, Score={box.score:.4f}", flush=True)
        #         print(f"    Box: [{real_box[0]:.1f}, {real_box[1]:.1f}, {real_box[2]:.1f}, {real_box[3]:.1f}]", flush=True)
        #         print(f"    Keypoints:", flush=True)
        #         for j in range(17):
        #             print(f"      KP {j}: x={real_kp[j][0]:.1f}, y={real_kp[j][1]:.1f}, conf={real_kp[j][2]:.4f}", flush=True)
        #     print("===================================================\n", flush=True)
...
@app.get("/")
async def index():
    # 从配置中获取 camera_id 和 video_path 状态
    config = det_config.get_config() if hasattr(det_config, 'get_config') else {}
    camera_id = config.get("camera_id", 1)
    video_path = config.get("video_path", None)
    
    # 动态生成标签页 HTML
    if video_path:
        # 当传入 video 参数时，只显示 Real-time Detection
        tabs_html = """
          <div class="tabs">
        ...
        # 只显示 Local Video Analysis
        tabs_html = """
          <div class="tabs">
            <div class="tab active" onclick="showTab('analysis')">Local Video Analysis</div>
          </div>
        """
        realtime_html = ""
    ...
    <html>
      ...
      <body>
        <div class="container">
          <h1>reComputer RK-CV Web Preview</h1>
          
          {tabs_html}
          {realtime_html}

          <div id="analysis" class="tab-content {analysis_active}" style="{analysis_style}">
        ...
        <script>
          function showTab(tabId) {{
            ...
            if (tabId === 'realtime') {{
                ...
                if (streamImg) streamImg.src = '/api/video_feed';
            }} else {{
                const streamImg = document.getElementById('streamImg');
...
# draw function removed as it is replaced by draw_pose
...
def main():
    ...
    # 通过检测配置传递 camera_id 和 video_path 状态，以便 Web 界面动态调整标签页
    det_config.update_camera_id(args.camera_id)
    det_config.update_video_path(args.video_path)
