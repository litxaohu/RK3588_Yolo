...
from py_utils.yolov5_seg_utils import post_process_seg, draw_seg, CLASSES, IMG_SIZE, SegHelper
...
class VideoAnalyzer:
    def __init__(self, model=None, co_helper=None, anchors=None):
        self.model = model
        ...
        self.seg_helper = SegHelper(enable_letter_box=True)

    def set_engine(self, model, co_helper, anchors):
        self.model = model
        self.co_helper = co_helper
        self.anchors = anchors
    ...
    def _process_video(self, input_path, output_path):
        ...
        try:
            while not self._stop_event.is_set():
                ...
                # 推理流程
                if self.model and self.co_helper and self.anchors:
                    processed_img = preprocess_frame(frame, self.co_helper, self.seg_helper)
                    outputs = self.model.run(processed_img)
                    
                    if outputs is not None:
                        obj, nms = det_config.get()
                        boxes, classes, scores, seg_img = post_process_seg(outputs, self.anchors, obj, nms)
                        if boxes is not None:
                            frame = draw_seg(frame, boxes, classes, scores, seg_img, self.seg_helper)
...
_global_anchors = None

_global_seg_helper = None

@app.post("/api/models/yolov5_seg/predict")
async def predict(
    ...
    if _global_model is None or _global_co_helper is None or _global_anchors is None:
    ...
    try:
        ...
        # 预处理
        input_img = preprocess_frame(img, _global_co_helper, _global_seg_helper)
        ...
        # 后处理
        boxes, classes, scores, seg_img = post_process_seg(outputs, _global_anchors, target_conf, target_iou)
        ...
        if boxes is not None:
            for i in range(len(boxes)):
                ...
                real_box = _global_seg_helper.get_real_box(box)
                real_mask = _global_seg_helper.get_real_mask(seg, img.shape)
                
                # 获取 mask 中所有为 1 的坐标，这部分不直接传给前端以节省带宽，前端如果需要可以直接要原图
                # 此处仅返回 bbox
                predictions.append({
                    "class": CLASSES[cl],
...
def preprocess_frame(frame, co_helper, seg_helper=None):
    if seg_helper is not None:
        img = seg_helper.letterbox_resize(frame.copy(), new_shape=(IMG_SIZE[1], IMG_SIZE[0]), bg_color=0)
    else:
...
def main():
    ...
    global _global_model, _global_co_helper, _global_anchors, _global_seg_helper
    
    # 初始化模型
    model = RKNNLiteModel(args.model_path)
    co_helper = COCO_test_helper(enable_letter_box=True)
    seg_helper = SegHelper(enable_letter_box=True)

    # 导出模型为全局变量并设置分析器
    _global_model = model
    _global_co_helper = co_helper
    _global_anchors = anchors
    _global_seg_helper = seg_helper
    video_analyzer.set_engine(model, co_helper, anchors)
    ...
    try:
        while True:
            ...
            # 推理流程
            processed_img = preprocess_frame(frame, co_helper, seg_helper)
            ...
            if outputs is not None:
                obj, nms = det_config.get()
                boxes, classes, scores, seg_img = post_process_seg(outputs, _global_anchors, obj, nms)
                if boxes is not None:
                    frame = draw_seg(frame, boxes, classes, scores, seg_img, seg_helper)
