...
def post_process_seg(input_data, anchors, obj_thresh=0.25, nms_thresh=0.45):
    ...
    seg_part_raw = [input_data[i*2+1].reshape([len(anchors[0]), -1]+list(input_data[i*2+1].shape[-2:])) for i in range(len(anchors))]
    proto = input_data[-1]
    ...
    seg_part = np.concatenate([sp_flatten(_v) for _v in seg_part_raw])
