...
# 导入共享工具
from py_utils.seg_utils import SegHelper, post_process_seg, draw_seg, CLASSES
...
def load_classes(path):
    pass # 如果需要动态加载类别，可以修改 seg_utils 中的 CLASSES
            ...
            while not self._stop_event.is_set():
                ...
                if self.model and self.co_helper:
                    ...
                    if outputs is not None:
                        ...
                        boxes, classes, scores, seg_img = post_process_seg(outputs, obj, nms)
                        if boxes is not None and len(boxes) > 0:
                            frame = draw_seg(frame, boxes, classes, scores, seg_img, self.co_helper)
...
async def predict(
    ...
    try:
        ...
        # 后处理
        boxes, classes, scores, seg_img = post_process_seg(outputs, target_conf, target_iou)
        ...
        if boxes is not None and len(boxes) > 0:
            ...
            for i in range(len(boxes)):
                ...
                # 提取多边形轮廓
                mask_uint8 = (real_mask * 255).astype(np.uint8)
                contours, _ = cv2.findContours(mask_uint8, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                
                polygons = []
                for contour in contours:
                    if len(contour) > 2:
                        poly = [{"x": float(pt[0][0]), "y": float(pt[0][1])} for pt in contour]
...
def preprocess_frame(frame, co_helper):
    img = co_helper.letterbox_resize(frame.copy(), new_shape=(IMG_SIZE[1], IMG_SIZE[0]), bg_color=114)
...
def main():
    ...
    co_helper = SegHelper(enable_letter_box=True)
    ...
    try:
        while True:
            ...
            if outputs is not None:
                ...
                boxes, classes, scores, seg_img = post_process_seg(outputs, obj, nms)
                if boxes is not None and len(boxes) > 0:
                    frame = draw_seg(frame, boxes, classes, scores, seg_img, co_helper)
